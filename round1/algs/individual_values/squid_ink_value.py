...
class Trader:
    ...
    def SQUID_INK_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
        ...
        # Initialize history and tick counter
        if "SQUID_INK_price_history" not in traderObject:
            traderObject["SQUID_INK_price_history"] = []
        if "SQUID_INK_tick" not in traderObject:
            traderObject["SQUID_INK_tick"] = 0

        tick = traderObject["SQUID_INK_tick"]
        price_history = traderObject["SQUID_INK_price_history"]

        # Append to history if price changed significantly
        if len(price_history) == 0 or abs(price_history[-1][0] - mmmid_price) >= 1:
            price_history.append((mmmid_price, tick))

        # Only keep events from last 60 ticks
        price_history = [(p, t) for p, t in price_history if tick - t <= 60]

        traderObject["SQUID_INK_price_history"] = price_history
        traderObject["SQUID_INK_tick"] += 1

        # Hawkes parameters
        mu = 0.1
        alpha = 0.9
        beta = 0.05  # slower decay because ticks are coarser than real time

        # Compute Hawkes intensity
        intensity = mu + sum(alpha * math.exp(-beta * (tick - t)) for _, t in price_history)

        # Use intensity to adjust fair value slightly
        fair = mmmid_price * (1 + 0.01 * (intensity - mu))
